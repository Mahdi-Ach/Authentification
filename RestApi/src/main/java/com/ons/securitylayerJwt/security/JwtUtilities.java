...
public class JwtUtilities{
    ...
    public String extractUsername(String token,boolean refresh_token) {
        if(refresh_token){
    ...
    public Claims extractAllClaimsRefreshToken(String token) {return Jwts.parser().setSigningKey(secretRefresh).parseClaimsJws(token).getBody();}
    ...
    public Boolean validateToken(String token, UserDetails userDetails) {
        final String email = extractUsername(token,false);
    ...
    public String generateToken(String email,String name , List<String> roles) {
        ...
        return Jwts.builder().setSubject(email).claim("role",roles).claim("name",name).setIssuedAt(new Date(System.currentTimeMillis()))
                .setExpiration(Date.from(Instant.now().plus(jwtExpiration, ChronoUnit.MILLIS)))
                .signWith(SignatureAlgorithm.HS256, secret).compact();
    }
    public String generateRefreshToken(String email, List<String> roles) {
        return Jwts.builder().setSubject(email).claim("role",roles).setIssuedAt(new Date(System.currentTimeMillis()))
                .setExpiration(Date.from(Instant.now().plus(refreshExpiration, ChronoUnit.MILLIS)))
                .signWith(SignatureAlgorithm.HS256, secretRefresh).compact();
    }
    public boolean validateRefreshToken(String token) {
        try {
            Jwts.parser().setSigningKey(secretRefresh).parseClaimsJws(token);
            return true;
